 ...
 * all its functionality from the abstract class BirdBrainJava.Robot. The only methods it needs here are constructors,
...
public class Microbit extends Robot {
	...
	/** This function tries to read sensor 4 (the BirdBrainJava.Hummingbird battery) to determine whether
     * the device is a BirdBrainJava.Hummingbird or a micro:bit. It returns true if the device is a micro:bit. */
    private boolean isMicrobit() {
		...
		if (stringResponse.equals("false")) {
			System.out.println("Error: Device " + deviceInstance + " is not a BirdBrainJava.Microbit");
			return false;
